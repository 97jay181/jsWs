...
public class Application {
    ...
    public static void main(String[] args) {
        ...
        /* 설명. Car라는 클래스를 하나 만든다.
         *  자동차의 역할을 수행할 수 있는 객체로, 객체지만, 기본적인 기능만 수행할 수 있다.
         * */
        ...
        /* 설명. FireCar 클래스는 아무것도 작성하지 않았는데도 Car 클래스가 가진 메소드를 모두 사용할 수 있다.
         *  그러나 이런 상속 관계에서도 private 멤버에는 접근할 수 없다.  */
        fireCar.isRunning();
//        fireCar.isRunning();

        /* 목차 3. 상속은 '확장(extends)' 이다. 부모에는 없는, 혹은 부모는 못하는 기능도 작성할 수 있다. */
        fireCar.sprayWater();

        /* 설명. 이렇게 상속은 부모가 가진 멤버를 사용하면서 확장까지 가능하도록 해둔 문법이다.
         *  상속은 자식이 부모 멤버에 접근해서 자신것처럼 사용 가능했지만, 반대의 경우는 허용하지 않는다.
         * */

        /* 목차 4. RacingCar 인스턴스 생성 후 메소드 호출해서 테스트 */
        System.out.println("==== 목차 4. =====");
        ...
        /* 설명. 레이싱카에 경적 소리를 내는 것이 적합하지 않다.
         *  하지만 Car를 상속받았기 때문에 어쩔수 없이 RacingCar도 경기장에서 경적을 울리게 된다.
         *  상속으로 인해 불필요한 기능이 추가될 수 있다는 점이 이런 부분이다.
         * ------------------------------------------------------------------------------------------
         *  자동차(Car)의 run() 메소드로는 레이싱카의 속도를 표현하기 부족하다.(= 추상적이다)
         *  따라서 일반 자동차보다 더 빠르게 달릴 수 있도록 오버라이딩 했다. (= 자식에게 구체적인 기능을 명시했다.)
         * */
