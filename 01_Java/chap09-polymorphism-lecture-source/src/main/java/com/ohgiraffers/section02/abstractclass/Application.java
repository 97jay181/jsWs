...
public class Application {
    ...
    public static void main(String[] args) {
        ...
        /* 설명. Product 클래스는 추상클래스이기 떄문에 인스턴스를 생성할 수 없다. */
//    Product prd = new Product() {

        /* 설명. 추상클래스를 상속받아 구현한 객체를 이용해야 한다. */
        SmartPhone smt = new SmartPhone();

        System.out.println(smt instanceof SmartPhone);
        System.out.println(smt instanceof Product);

        /* 설명. 다형성을 적용해서 추상클래스를 레퍼런스 타입으로 활용할 수도 있다.*/
        Product prd = new SmartPhone();

        /* 설명. 동적 바인딩에 의해 실제 런타임에는 SmartPhone의 메소드가 호출된다. */
        prd.absMethod();

        /* 설명. static 메서드는 바로 사용 가능(추상클래스임에도 불구하고 여전히 인스턴스 생성 불필요) */
        prd.staticMethod();

        /* 설명. 추상클래스를 사용하는 이유?
         *  추상클래스의 추상메서드는 오버라이딩에 대한 강제성이 부여된다.
         *  따라서 여러 클래스를 그룹화하여 필수기능을 정의, 그리고 강제성을 부여해 개발시 일관된 인터페이스를 제공할 수 있다.
         * ====================================================================================================================================
         * 하지만 다른 클래스를 상속받고 있는 클래스를 작성할 때에는 추상클래스를 추가로 상속받을 수 없다.
         * 그래서 Java는 추상클래스보다 더 강제성이 강한 인터페이스(interface)라는 메커니즘을 제공하고 있다.
         * */
