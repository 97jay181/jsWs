...
public class Application2 {
    ...
    public static void main(String[] args) {
        ...
        /* 수업목표. 변수를 선언(declare)하고, 값을 할당(assign)하여 사용(call)할 수 있다.*/
        /* 필기.
        *   변수를 사용하는 순서.
        *    1. 변수를 준비한다 -> 선언
        *    2. 변수에 값을 대입한다 -> 값 대입 또는 초기화
        *    3. 변숙를 사용한다 -> 호출
        * */

        /* 목차. 1. 변수를 준비한다 (선언; declare) */
        /* 목차. 1-1. 변수를 선언하는 방법 */
        /* 필기. 자료형 변수명; */

        /* 필기. 자료형(data type)이란?
        *   다양한 값의 형태별로 어느 정도의 크기를 하나의 값으로 취급할 것인지 미리 컴파일러(compiler)와 약속한 키워드
        ...
        * */
        ...
        /* 설명. 이상, 위에서 다룬 8가지 기본 자료형(Primary Type)이라고 한다. */

        /* 설명.
        *   앞에서 다뤄본 값의 형태 중, 문자열이 빠져있다.
        *   문자열은 조금 다른 방식으로 취급하게 되는데, 이 부분은 나중에 더 깊게 다루게 될 것이다.
        * */
        ...
        /* 설명.
        *   위에서 한 변수 선언은 메모리에 값을 저장하기 위해 공간만 생성해둔 상태다.
        *   그 공간에 대입 연산자를 ㅣ용하여 자료형에 저장하기로 한 형태의 값을 저장할 수 있다.
        *   만약, 약속 내용과 다른 형태의 값을 대입하려고 하면 컴 파일러는 에러를 발생시킨다.
        *   ---------------------------------------------------------------------------------------
        *   대임연산자의 실행 방향은 오른쪽에서 왼쪽이다.
        *   즉, 오른쪽에 있는 값을 왼쪽의 공간에 대입한다는 의미이며, 왼쪽에는 항상 공간, 오른쪽에는 항상 값이 위치한다.
